...
import { TEMPLATES, DROPDOWN_OPTIONS } from './templates-config.js';
...
function expandGoogleDocTable(text) {
  ...
  for (const line of lines) {
    ...
    // shared body script into this creative so all variations get it.
    ...
    if (mode === 'bodyscript') {
      if (trimmed === '') { mode = 'normal'; continue; } // blank line ends the section
      sharedBody.push(trimmed);
      continue;
    }
...
function collectDropdownWarnings(briefType, overview, creatives) {
  ...
  // Overview-scope dropdowns
  for (const def of cfg.overview) {
    if (def.kind !== 'dropdown') continue;
    const val = overview[def.field];
    if (val == null || val === '') continue;
    const opts = DROPDOWN_OPTIONS[def.options] || [];
    if (matchesOption(val, opts)) continue;
    warnings.push({
      scope: 'overview',
      creativeIndex: null,
      field: def.field,
  ...
  // Creative-scope dropdowns (per creative)
  creatives.forEach((c, idx) => {
    for (const def of cfg.creative) {
      if (def.kind !== 'dropdown') continue;
      const val = c[def.field];
      if (val == null || val === '') continue;
      const opts = DROPDOWN_OPTIONS[def.options] || [];
      if (matchesOption(val, opts)) continue;
      warnings.push({
