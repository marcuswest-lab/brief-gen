...
// If the pasted text is HTML (the brief is generated as an .html file), convert
// it to the canonical line shapes the rest of the pipeline understands:
//   - <h1..h6> headings  -> their own text line (section breaks / "Video 1 — ...")
//   - 2-cell <tr> rows   -> "Field<TAB>Value" (single-line values, placeholder-aware)
//                           or "Field: line1" + continuation lines (multi-line cells)
// Handles both raw markup pastes and is a no-op for plain text. Browser-only
// (uses DOMParser); falls back to the original text if DOMParser is unavailable.
function htmlToText(text) {
  if (!/<(table|tr|td|th|h[1-6]|body|div|p)\b/i.test(text)) return text;
  ...
  // Read an element's text: <br> -> newline, entities decoded, whitespace tidied.
  const cellText = (el) => {
  ...
  const out = [];
  const emitRow = (key, value) => {
    if (!value) { out.push(`${key}:`); return; }
    if (value.includes('\n')) {
      // Multi-line cell: colon form keeps legit bracketed stage directions in the
      // value (only single-line tab rows get placeholder-blanked downstream).
      const lns = value.split('\n');
      out.push(`${key}: ${lns[0]}`);
      for (let i = 1; i < lns.length; i++) out.push(lns[i]);
    } else {
      out.push(`${key}\t${value}`);
    }
  };
    ...
    for (const el of node.children) {
      ...
      if (/^h[1-6]$/.test(tag)) {
        ...
        for (const row of el.querySelectorAll('tr')) {
          ...
          if (cells.length >= 2) emitRow(cells[0], cells[1]);
